    ...
    def _handle_tool_execution(self, initial_response, base_params: Dict[str, Any], tool_manager):
        ...
        while round_count < self.MAX_TOOL_ROUNDS:
            ...
            try:
                ...
                print(f"[AI Generator] Round {round_count} completed, stop_reason: {current_response.stop_reason}")
            except Exception as e:
                error_msg = f"Error in tool execution round {round_count}: {str(e)}"
                print(f"[AI Generator] {error_msg}")
                return self._extract_text_content(current_response) if current_response else error_msg
        
        # Maximum rounds reached - return final response
        print(f"[AI Generator] Maximum rounds ({self.MAX_TOOL_ROUNDS}) reached")
        return self._extract_text_content(current_response)
    
    def _execute_all_tools(self, response, tool_manager, round_count: int) -> Tuple[List[Dict], bool]:
        """
        ...
        try:
            for content_block in response.content:
                if content_block.type == "tool_use":
                    print(f"[AI Generator] Round {round_count}: Executing {content_block.name} with {content_block.input}")
