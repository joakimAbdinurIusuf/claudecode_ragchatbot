...
class MockToolUseContent:
    ...
    def __init__(
        self, tool_name="search_course_content", tool_input=None, tool_id="mock-tool-id"
    ):
        self.type = "tool_use"
        self.name = tool_name
...
class TestAIGenerator:
    ...
    def test_generate_response_with_tool_use(self, mock_anthropic):
        ...
        initial_response = MockAnthropicResponse(
            stop_reason="tool_use", tool_use_content=tool_use_content
        )
...
class TestSequentialToolCalling:
    ...
    def test_sequential_two_round_tool_calling(self, mock_anthropic):
        ...
        round1_response = MockAnthropicResponse(
            stop_reason="tool_use", tool_use_content=round1_tool_use
        )
        ...
        round2_response = MockAnthropicResponse(
            stop_reason="tool_use", tool_use_content=round2_tool_use
        )

        # Final response
        final_response = MockAnthropicResponse(
            "Based on the course outline and specific search, neural networks are..."
        )
    ...
    def test_max_rounds_enforcement(self, mock_anthropic):
        ...
        assert (
            "Mock AI response" in response
            or "couldn't generate a text response" in response
            or "tool use" in response.lower()
        )
    ...
    def test_tool_error_handling_mid_sequence(self, mock_anthropic):
        ...
        assert (
            "Mock search error" in response
            or "error" in response.lower()
            or "couldn't generate" in response.lower()
        )
